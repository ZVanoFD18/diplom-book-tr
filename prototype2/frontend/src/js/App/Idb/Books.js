		...
		return new Promise((resolve, reject) => {
			document.App.Idb.getDb().then((db) => {
				...
				req.onerror = (event) => {
					...
					reject(new document.App.Errors.User(document.App.localize('Ошибка при поиске книги по хешу.')));
				};
			});
		});
	}
		...
		return new Promise((resolve, reject) => {
			...
			document.App.Idb.getDb().then((retDb) => {
				...
				return new Promise((resolve, reject) => {
					...
					req.onerror = (event) => {
						document.Helper.Log.addDebug(event);
						reject(new document.App.Errors.User(document.App.localize('Не удалось добавить книгу в БД.')));
					};
					// @TODO: удалить блок после исследования.
					transaction.oncomplete = function (e) {
						console.log(' document.App.Idb.Books.add/transaction.oncomplete', e);
						db.close();
					};
				})
			}).then((putResult) => {
